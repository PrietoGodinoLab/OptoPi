...
void loop() 
{
    ...
    //Read the state (0 or 1) from serial sent from the Raspberry Pi
    //Blink_from_Pi_serial();

    //Read the RGB matrix intensity value(0 to 255) from serial sent from the Raspberry Pi
    //Custom_intensity_from_Pi_serial();
...
//      Serial.println(millis());
  ...
  void Custom_intensity_from_Pi_serial()
  {
    value  = Serial.parseInt();
    int intensity = constrain(value, 0, 255);
    Set_Matrix_Color_Brightness(red, green, blue, intensity);
    //delay(1000);
